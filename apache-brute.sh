...
zgrep -v dummy /var/log/apache2/access.* | awk -F ":\|\ -" '{print $2}' 2>/dev/null | sort | uniq -c | sort -rn | head -20 | awk -F " " '{print $2}' > top20.txt; while read line; do echo $line `geoiplookup $line`; done < top20.txt
